...
if(hero && !window.matchMedia('(prefers-reduced-motion: reduce)').matches){
 ...
 fx.innerHTML=`
  <span class="core-aura"></span><span class="core-scan"></span>
  <span class="vertical-core-beam beam-one"></span><span class="vertical-core-beam beam-two"></span>
  ...
  <span class="glow-node node-a"></span><span class="glow-node node-b"></span><span class="glow-node node-c"></span><span class="glow-node node-d"></span><span class="glow-node node-e"></span>
  ...
  <span class="spark spark-a"></span><span class="spark spark-b"></span><span class="spark spark-c"></span><span class="spark spark-d"></span>`;
 hero.appendChild(fx);
 const sparks=[...fx.querySelectorAll('.spark')];
 const randomizeSpark=s=>{s.style.left=(45+Math.random()*48)+'%';s.style.top=(8+Math.random()*82)+'%';s.style.setProperty('--spark-delay',(Math.random()*1.4)+'s');s.style.setProperty('--spark-scale',(0.65+Math.random()*0.9).toFixed(2));};
 sparks.forEach(randomizeSpark);setInterval(()=>sparks.forEach(randomizeSpark),3200);
}